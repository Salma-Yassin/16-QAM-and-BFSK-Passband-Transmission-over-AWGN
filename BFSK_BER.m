function BER = BFSK_BER(Eb,N0)


Tb = 5; %% bit duration in secs  
N_bit = 200; %%number of samples per bit 
...
for i= 1:1:L
    for j = i:1:L
        d_ij = sqrt((si_vector(1,i) - si_vector(1,j))^2 + (si_vector(2,i) - si_vector(2,j))^2);
        if d_ij < dmin && d_ij ~= 0
            dmin = d_ij;
        end
    end
